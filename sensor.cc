...
absl::Status RunRestContinuousEdge(
    ...
    // Counters for on-screen display (increment when detected, only when 1 face present)
    int total_blink_count = 0;
    int total_talk_count = 0;
    // Previous state for rising-edge detection (only count on false -> true transition)
    bool prev_blink_detected = false;
    bool prev_talk_detected = false;
    ...
    MP_RETURN_IF_ERROR(container.SetOnCoreMetricsOutput(
        [&settings, &hud, &enable_hud, &save_core_metrics_to_disk, &output_directory,
         &total_blink_count, &total_talk_count, &prev_blink_detected, &prev_talk_detected](
            ...
            if (metrics_buffer.has_face()) {
                const auto& face = metrics_buffer.face();

                // Blink: rising edge (false -> true)
                bool cur_blink = false;
                for (const auto& blink : face.blinking()) {
                    if (blink.detected()) { cur_blink = true; break; }
                }
                if (cur_blink && !prev_blink_detected) {
                    total_blink_count++;
                    std::cout << "[BLINK] Total blinks: " << total_blink_count << std::endl;
                }
                prev_blink_detected = cur_blink;

                // Talking: rising edge (false -> true)
                bool cur_talk = false;
                for (const auto& talk : face.talking()) {
                    if (talk.detected()) { cur_talk = true; break; }
                }
                if (cur_talk && !prev_talk_detected) {
                    total_talk_count++;
                    std::cout << "[TALKING] Total talking events: " << total_talk_count << std::endl;
                }
                prev_talk_detected = cur_talk;

            } else {
                std::cout << "No face data in metrics buffer" << std::endl;
            }
            ...
            if (settings.verbosity_level > 1) {
                ...
                std::cout << metrics_output.str() << std::endl;
            }
            if (enable_hud) {
    ...
    if (enable_hud) {
        MP_RETURN_IF_ERROR(container.SetOnVideoOutput(
            [&hud, &enable_edge_metrics, &hud_portrait_mode, &enable_micromotion,
                ...
                &accumulated_metrics, &total_blink_count, &total_talk_count]
                (cv::Mat& output_frame, int64_t timestamp_milliseconds) {
                auto status = hud.Render(output_frame);
                if (!status.ok()) { return status; }

                // Draw blink and talk counters on screen (top-left corner)
                cv::putText(output_frame,
                    "Blinks: " + std::to_string(total_blink_count),
                    cv::Point(20, 40),
                    cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
                cv::putText(output_frame,
                    "Talking: " + std::to_string(total_talk_count),
                    cv::Point(20, 80),
        ...
        MP_RETURN_IF_ERROR(container.SetOnEdgeMetricsOutput(
            [&settings, &hud_portrait_mode, &enable_micromotion, &save_edge_metrics_to_disk, &output_directory, &accumulated_metrics,
             ...
             (const presage::physiology::Metrics& metrics) {
                ...
                // --- Access Edge Face Metrics ---
                if (!metrics.has_face()) {
                    std::cout << "[WARNING] No face data in edge metrics!" << std::endl;
                }
        ...
        MP_RETURN_IF_ERROR(container.SetOnFrameSentThrough(
            [](
                ...
                if (!frame_sent_through) {
                    std::cout << "Dropped frame at timestamp " << timestamp_microseconds << std::endl;
                }
                return absl::OkStatus();
            }
        ));
...
int main(int argc, char** argv) {
    ...
    if (!status.ok()) {
        std::cerr << "[ERROR] " << "Run failed. " << status.message() << std::endl;
        return EXIT_FAILURE;
    } else {
